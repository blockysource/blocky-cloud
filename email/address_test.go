...
func TestEmptyAddress(t *testing.T) {
	...
	if len(list) > 0 || err == nil {
		t.Errorf(`ParseList("") = %v, %v, want nil, error`, list, err)
	}
	list, err = ParseAddressList(",")
	if len(list) > 0 || err == nil {
		t.Errorf(`ParseList("") = %v, %v, want nil, error`, list, err)
...
func BenchmarkParseAddress(b *testing.B) {
	test := `Böb O'Connor <bob@connor.org>`
	b.Run("Package.mail", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
	...
	b.Run("Package.email", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			_, err := ParseAddress(test)
			if err != nil {
				b.Fatal(err)
...
func BenchmarkVerifyAddress(b *testing.B) {
	test := `Böb O'Connor <bob@connor.org>`
	b.Run("Package.mail", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			_, err := mail.ParseAddress(test)
	...
	b.Run("VerifyAddressFormat", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			err := VerifyAddressFormat(test)
			if err != nil {
				b.Fatal(err)
			}
		}
...
func BenchmarkVerifyAddressList(b *testing.B) {
	test := `Böb O'Connor <bob@connor.org>, Alice <alice@eve.org>`
	b.Run("Package.mail", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			_, err := mail.ParseAddressList(test)
			if err != nil {
				b.Fatal(err)
			}
		}
	...
	b.Run("VerifyAddressFormat", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			err := VerifyAddressListFormat(test)
			if err != nil {
				b.Fatal(err)
			}
		}
